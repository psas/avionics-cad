...
h = 0.072 * 0.0254 # [m] Height of dielectric
min_padding = 3 * h
padding = 5 * h
...
l_holefromedge = 0.100 * 0.0254 # [m] Dist. From edge to hole

l_paddingtop = 0.300 * 0.0254   # [m] Padding from patch to edge - Makes the screws heads 5h away from the patch slot	
l_paddingbot = 0.300 * 0.0254   # [m] padding from via to edge - Makes the screws heads 5h away from the via	
l_paddingwidth = 0.010 * 0.0254 # [m] padding on patch width - Extra slop that we can sand off while making the antenna	
...
L  = pi *(airframe_od + H) # [m] FIXME Doesn't include t_pcb since the copper is on the *inside*
...
patch_diameter =  airframe_od + 2 * t_gnd + 2 * h # [m] diameter at the patch radiator
patch_width    = pi * patch_diameter # [m] Width of radiator
...
def eagle_rect (x1, y1, x2, y2):
	script.write('rect (' + str(x1*1000) + ' ' + str(y1*1000) + ') (' + str(x2*1000) + ' ' + str(y2*1000) + ');\n')
...
def draw_feed(level, feed, y, z_in, z_out, z_next):
	...
	# Corner: make a small rectangle to fill the space of the corner
	eagle_rect(x-x_offset, y, x-x_offset-y_width, y-y_width)
	eagle_rect(x+x_offset, y, x+x_offset+y_width, y-y_width)
	# Corner: make the 45 degree piece
	eagle_wire(y_width, x-x_offset, y+y_width/2, x-x_offset-y_width*1.5, y-y_width)
	eagle_wire(y_width, x+x_offset, y+y_width/2, x+x_offset+y_width*1.5, y-y_width)

	# Zout microstrip at 90 degrees
	x_offset2 = strip_width(z_out)
	y_offset = -y_width
	y_offset2 = -(min_padding + strip_width(z_next)/2)
...
try:  
	...
	eagle_wire(0.001, -x_cutting_guide, feed_top, -x_cutting_guide, y + patch_length + min_padding)
	# Draw outline
	eagle_wire(0.0002, -x_outer_holes - 0.010, top_row + 0.010, -x_outer_holes - 0.010, bottom_row - 0.01)
	eagle_wire(0.0002, -x_outer_holes - 0.010, top_row + 0.010, x_outer_holes + 0.010, top_row + 0.010)
	eagle_wire(0.0002, x_outer_holes + 0.010, top_row + 0.010, x_outer_holes + 0.010, bottom_row - 0.01)
	eagle_wire(0.0002, -x_outer_holes - 0.010, bottom_row - 0.010, x_outer_holes + 0.010, bottom_row - 0.010)
